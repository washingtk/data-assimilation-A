...
program main
    ...
    !make directory
    !
    write(dirname, '("output_kf_draw/")')
    call makedirs(dirname)
    write(filename, '("kf_out_alpha", i2.2, f0.3, ".txt")') &
        & int(alpha), alpha-int(alpha)
    ...
    do t_idx = 1, t_1year
        ...
        XX = XX_NEX
        call runge_kutta_method(NN, XA, XF)
        XA = XF
    end do
